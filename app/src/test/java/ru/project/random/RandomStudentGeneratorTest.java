...
public class RandomStudentGeneratorTest {
  ...
  void shouldTestgenerate_withoutConstraints_returnsCorrectSizeAndValidStudents() {
    ...
    for (int i = 0; i < students.size(); i++) {
      ...
      assertTrue(
          GROUP_PATTERN.matcher(s.getGroupNumber()).matches(),
          "groupNumber должен соответствовать шаблону A00–Z99");
      assertTrue(
          s.getAverageGrade() >= MIN_GRADE && s.getAverageGrade() <= MAX_GRADE,
          "averageGrade должен быть в диапазоне [0.0, 5.0]");
      assertTrue(s.getRecordBookNumber() >= 1, "recordBookNumber должен быть >= 1");
    }
  }
  ...
  void shouldTestgenerate_withGroupRange_respectsLexicographicalBounds() {
    RandomStudentGenerator generator =
        new RandomStudentGenerator().setMinGroupNumber("A00").setMaxGroupNumber("B99");
  ...
  void shouldTestgenerate_withGradeRange_respectsBounds() {
    ...
    RandomStudentGenerator generator =
        new RandomStudentGenerator().setMinAverageGrade(minGrade).setMaxAverageGrade(maxGrade);
    MyList<Student> students = generator.generate(100);

    for (int i = 0; i < students.size(); i++) {
      Student s = students.get(i);
      double g = s.getAverageGrade();
      assertTrue(g >= minGrade && g <= maxGrade, "averageGrade должен быть в заданном диапазоне");
    }
  }
  ...
  void shouldTestgenerate_withRecordBookRange_respectsBounds() {
    ...
    RandomStudentGenerator generator =
        new RandomStudentGenerator().setMinRecordBookNumber(minRec).setMaxRecordBookNumber(maxRec);
    MyList<Student> students = generator.generate(100);

    for (int i = 0; i < students.size(); i++) {
      Student s = students.get(i);
      int r = s.getRecordBookNumber();
      assertTrue(r >= minRec && r <= maxRec, "recordBookNumber должен быть в заданном диапазоне");
    }
  }
  ...
  void shouldTestsetMinGroupNumber_invalidPattern_throwsException() {
    RandomStudentGenerator generator = new RandomStudentGenerator();
    assertThrows(IllegalArgumentException.class, () -> generator.setMinGroupNumber("invalid"));
  }
  ...
  void shouldTestsetMaxGroupNumber_invalidPattern_throwsException() {
    RandomStudentGenerator generator = new RandomStudentGenerator();
    assertThrows(
        IllegalArgumentException.class, () -> generator.setMaxGroupNumber("ZZ")); // не 2 цифры
  }
  ...
  void shouldTestsetMinAverageGrade_outOfRange_throwsException() {
    RandomStudentGenerator generator = new RandomStudentGenerator();
    assertThrows(IllegalArgumentException.class, () -> generator.setMinAverageGrade(-0.1));
    assertThrows(IllegalArgumentException.class, () -> generator.setMinAverageGrade(5.1));
  }

  @Test
  void shouldTestsetMaxAverageGrade_outOfRange_throwsException() {
    RandomStudentGenerator generator = new RandomStudentGenerator();
    assertThrows(IllegalArgumentException.class, () -> generator.setMaxAverageGrade(-0.1));
    assertThrows(IllegalArgumentException.class, () -> generator.setMaxAverageGrade(5.1));
  }
  ...
  void shouldTestsetMinRecordBookNumber_lessThanAllowed_throwsException() {
    RandomStudentGenerator generator = new RandomStudentGenerator();
    assertThrows(IllegalArgumentException.class, () -> generator.setMinRecordBookNumber(0));
  }
  ...
  void shouldTestchainedBuilderStyle_worksAsExpected() {
    ...
    for (int i = 0; i < students.size(); i++) {
      Student s = students.get(i);
      assertTrue(
          s.getGroupNumber().compareTo("A00") >= 0 && s.getGroupNumber().compareTo("Z99") <= 0);
